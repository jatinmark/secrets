
import React from 'react';
import './LowerContent.css';
import MoreHorizIcon from '@mui/icons-material/MoreHoriz';
import AddIcon from '@mui/icons-material/Add';
import { experimentalStyled as styled } from '@mui/material/styles';
import Box from '@mui/material/Box';
import Paper from '@mui/material/Paper';
import Card from '@mui/material/Card';
import CardActions from '@mui/material/CardActions';
import CardContent from '@mui/material/CardContent';
import CardMedia from '@mui/material/CardMedia';
import Button from '@mui/material/Button';
import Typography from '@mui/material/Typography';
import Grid from '@mui/material/Unstable_Grid2';

function LowerContent() {


  const Item = styled(Paper)(({ theme }) => ({
    backgroundColor: theme.palette.mode === 'dark' ? '#1A2027' : '#fff',
    ...theme.typography.body2,
    padding: theme.spacing(2),
    textAlign: 'center',
    color: theme.palette.text.secondary,
  }));
  



  return (
    <div>
    <div className="lower_content">
   
   <div className="categories">
   <button className='button'  size="small"  ><strong><p className='barsize'>To Do Task</p></strong><AddIcon sx={{width: 20 ,height :20 ,color:"rgb(214, 214, 214)" , margin: "0px 4px"}} />< MoreHorizIcon sx={{width: 20 ,height :20 ,color:"rgb(214, 214, 214)"}}  /></button>

 </div>
 <div className="categories">
   <button className='button'  size="small"  ><strong><p className='barsize'>In Progress</p></strong><AddIcon sx={{width: 20 ,height :20 ,color:"rgb(214, 214, 214)" , margin: "0px 4px"}} />< MoreHorizIcon sx={{width: 20 ,height :20 ,color:"rgb(214, 214, 214)"}}  /></button>

 </div>
 <div className="categories">
   <button className='button'  size="small"  ><strong><p style={{color : 'rgb(153, 151, 151)' ,margin: "6px 30px 6px 10px"}}>Waiting For Feedback</p></strong><AddIcon sx={{width: 20 ,height :20 ,color:"rgb(214, 214, 214)" , margin: "0px 4px"}} />< MoreHorizIcon sx={{width: 20 ,height :20 ,color:"rgb(214, 214, 214)"}}  /></button>

 </div>
 <div className="categories">
   <button className='button'  size="small"  ><strong><p className='barsize'>Done</p></strong><AddIcon sx={{width: 20 ,height :20 ,color:"rgb(214, 214, 214)" , margin: "0px 4px"}} />< MoreHorizIcon sx={{width: 20 ,height :20 ,color:"rgb(214, 214, 214)"}}  /></button>

 </div>


    
    </div>
    <Box sx={{ flexGrow: 1 ,  margin: "24px 34px 0 28px"}}>
      <Grid container spacing={{ xs: 2, md: 4 }} columns={{ xs: 4, sm: 8, md: 12 }}>
        {Array.from(Array(8)).map((_, index) => (
          <Grid xs={2} sm={2} md={3} key={index}>
            <Item>
              <Card >
      <CardMedia
        component="img"
        height="20"
        image="data:image/jpeg;base64,"
        alt="green iguana"
      />
      <CardContent>
        <Typography gutterBottom variant="h5" component="div">
          
        </Typography>
        <Typography variant="body2" color="text.secondary">
        
        </Typography>
      </CardContent>
      <CardActions>
        <Button size="small">Share</Button>
        
      </CardActions>
    </Card>
    </Item>
          </Grid>
        ))}
      </Grid>
    </Box>
   

    </div>
  );
}

export default LowerContent;